...
public class CompareExcelContent {
    public static void main(String[] args) {
        ...
        try (FileInputStream file1 = new FileInputStream(file1Path);
            ...
            boolean areEqual = compareSheetContents(sheet1, sheet2);
            if (areEqual) {
                System.out.println("Sheet contents are identical.");
            } else {
                System.out.println("Sheet contents are not identical 2");
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private static boolean compareSheetContents(Sheet sheet1, Sheet sheet2) {
        ...
        for (int i = sheet1.getFirstRowNum(); i <= sheet1.getLastRowNum(); i++) {
            ...
            for (int j = row1.getFirstCellNum(); j < row1.getLastCellNum(); j++) {
                ...
                if (!areCellValuesEqual(cell1, cell2)) {
                    return false; // Cell values are different
                }
            }
        }

        return true; // Sheet contents are identical
    }

    private static boolean areCellValuesEqual(Cell cell1, Cell cell2) {
